...
st.markdown("3. Validation and testing are performed by using a walk-forward validation approach, by training the saved models using the previous day's data to predict the current day's net load demand.")
st.markdown("4. The process is repeated for every forecasted day.")
...
st.markdown("2. The weather data is assumed to be accurate and reliable. For the purpose of this tool, the forecasted weather data is assumed to the next day's observed data. In the future, the tool will be updated to use forecasted weather data.")
...
st.markdown("7. The tool is currently limited to forecasting for the next day. The tool was built dynamically to be easy to update the forecast horizon for few hours ahead or multiple days ahead based on requirement and data availability. The model will need to be retrained for different forecast horizons.")
